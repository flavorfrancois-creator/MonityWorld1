...
import { Phone, User, DollarSign, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function PartnerRecharge() {
  ...
  const handleRecharge = async () => {
    ...
    setResult(null);
    try {
      const res = await API.post('/partner/client/recharge', {
      ...
      // Update stored partner balance and emit event for sidebar update
    ...
    <div className="p-6 max-w-lg mx-auto space-y-6">
      <div className="animate-fade-in-up">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Transférez des fonds vers le compte d'un client
        </p>
      </div>
      ...
      {result && (
        <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 animate-fade-in-up">
          <div className="flex items-center gap-2 text-green-400 mb-3">
            ...
            <span className="font-medium">Recharge réussie</span>
          </div>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-red-400">-${result.client_fee}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Client reçoit</span>
              <span className="text-green-400">${result.client_receives}</span>
            </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4 animate-fade-in-up stagger-1">
        ...
        >
          {loading ? 'Traitement...' : 'Effectuer la recharge'}
