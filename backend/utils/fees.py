"""
Monity World - Fee & Transaction Rules Utilities
"""
...
async def get_transaction_rule(country_code: str, tx_type: str):
    rule = await db.transaction_rules.find_one({
        "country_code": country_code, "transaction_type": tx_type, "is_active": True
    })
    if not rule:
        rule = await db.transaction_rules.find_one({
            "country_code": "ALL", "transaction_type": tx_type, "is_active": True
        })
    if not rule:
        rule = {
            "fee_type": "percentage",
            "fee_value": 1.0 if tx_type == "transfer" else 1.5,
            "min_fee": 0.0, "max_fee": None,
            "daily_limit": 10000, "monthly_limit": 100000,
            "per_transaction_min": 1, "per_transaction_max": 5000
        }
    return rule
...
async def get_international_rule(source_country: str, dest_country: str):
    rule = await db.international_rules.find_one({
        "source_country": source_country, "destination_country": dest_country, "is_active": True
    })
    if not rule:
        rule = await db.international_rules.find_one({
            "source_country": dest_country, "destination_country": source_country, "is_active": True
        })
    if not rule:
        rule = await db.international_rules.find_one({
            "source_country": "ALL", "destination_country": "ALL", "is_active": True
        })
    if not rule:
        rule = {
            "fee_type": "percentage", "fee_value": 2.5, "min_fee": 1.0, "max_fee": None,
            "exchange_rate_margin": 2.0, "daily_limit": 5000, "monthly_limit": 50000,
            "per_transaction_min": 10, "per_transaction_max": 2000, "processing_time": "instant"
        }
    return rule
...
async def check_transaction_limits(user_id: str, amount: float, tx_type: str, rule: dict) -> dict:
    ...
    daily_total = await db.transactions.aggregate([
        {"$match": {"sender_id": user_id, "type": tx_type, "status": {"$in": ["completed", "pending"]}, "created_at": {"$gte": today_start.isoformat()}}},
    ...
    monthly_total = await db.transactions.aggregate([
        {"$match": {"sender_id": user_id, "type": tx_type, "status": {"$in": ["completed", "pending"]}, "created_at": {"$gte": month_start.isoformat()}}},
        {"$group": {"_id": None, "total": {"$sum": "$amount"}}}
    ]).to_list(1)
    ...
    return {
        "allowed": len(errors) == 0, "errors": errors,
        "daily_spent": daily_spent, "daily_remaining": max(0, rule.get("daily_limit", 0) - daily_spent),
        "monthly_spent": monthly_spent, "monthly_remaining": max(0, rule.get("monthly_limit", 0) - monthly_spent)
    }
